...
lap = np.uint8(np.absolute(lap)) #using numpy to take max value of lap and convert into 8 bit integer
...
for i in range(4):
    plt.subplot(2, 2, i+1), plt.imshow(images[i], 'gray') #creating a 2 by 3 subplot as in matlab
    plt.title(titles[i])
